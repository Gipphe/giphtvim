...
nixInfo.lze.register_handlers {
  ...
  },
  {
    ...
    modify = function(plugin)
      if vim.g.nix_info_plugin_name then
        if type(plugin.for_cat == 'function') then
          local cats = nixInfo({}, 'settings', 'cats')
          plugin.enabled = plugin.for_cat(cats)
          return plugin
        end

        if type(plugin.for_cat == 'string') then
          plugin.for_cat = { plugin.for_cat }
        end
        if type(plugin.for_cat) == 'table' and vim.islist(plugin.for_cat) then
          for _, cat in pairs(plugin.for_cat) do
            plugin.enabled = nixInfo(false, 'settings', 'cats', cat)
