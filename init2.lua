...
if not nixInfo.isNix then
  vim.pack.add({ { name = 'lze', src = util.gh 'BirdeeHub/lze' } }, { confirm = false })
end
...
--- Loads plugins using `vim.pack.add`, and then load them with `lze`
--- afterwards. If in nix, this will just load them with `lze`.
---@param specs Spec[]
local function load_plugins(specs)
  local enabled_specs = {}
  for _, spec in ipairs(specs) do
    if type(spec.enabled) ~= 'boolean' or spec.enabled then
      table.insert(enabled_specs, spec)
    end
  end

  if nixInfo.isNix then
    local res = {}
    for _, p in pairs(enabled_specs) do
      local spec = p.spec.data or {}
      spec.name = p.spec.name
      res[#res + 1] = spec
    end
    nixInfo.lze.load(res)
  else
    local pack_specs = {}
    local lze_specs = {}
    for _, p in pairs(enabled_specs) do
      local pack_spec = util.omit(p, 'data')
      local lze_spec = p.data
      lze_spec.name = p.name
      pack_specs[#pack_specs + 1] = pack_spec
      lze_specs[#lze_specs + 1] = lze_spec
    end
    vim.pack.add(lze_specs, { confirm = true })
    nixInfo.lze.load(lze_specs)
  end
end
        ...
        lazygit = {
          config = {
            os = {
              ...
              -- this one isn't a remote command, make sure it gets our config regardless of if we name it nvim or not
